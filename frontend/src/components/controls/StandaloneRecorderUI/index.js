...
class StandaloneRecorderUI extends Component {
  ...
  startRecording = (evt) => {
    ...
    const { sessionNotes, url, filter, level } = this.state;
  ...
  render() {
    ...
    const { advOpen, initialOpen, url, filter, level } = this.state;
        ...
        <FormGroup className={classNames('col-md-8 col-md-offset-2', { 'input-group': extractable })} validationState={this.state.validation}>
          ...
          <div className="row" style={{marginTop: '15px'}}>
            <div className="col-md-6">
              <FormControl id="filter" aria-label="filter" type="text" name="filter" onChange={this.handleInput} style={{ height: '33px' }} value={filter} placeholder="Filter" title={isOutOfSpace ? 'Out of space' : 'Enter filter string'} required disabled={isOutOfSpace} />
            </div>
            <div className="col-md-6">
              <FormControl id="level" aria-label="level" type="number" name="level" onChange={this.handleInput} style={{ height: '33px' }} value={level} placeholder="Level" title={isOutOfSpace ? 'Out of space' : 'Enter filter string'} required disabled={isOutOfSpace} />
            </div>
          </div>
          <ExtractWidget
