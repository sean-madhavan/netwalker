...
class TableRenderer extends Component {
  ...
  render() {
    const { canAdmin } = this.context;
    const { activeList, browsers, collection, list, selectedPageIdx, displayObjects } = this.props;
    let filteredDisplayObjects = List();
    const filter = localStorage.getItem('filter');
    const level = parseInt(localStorage.getItem('level'), 10);
    filteredDisplayObjects = displayObjects.filter(displayObject => displayObject.get('url').indexOf(filter) !== -1 && (displayObject.get('url').split('/').length - 2) <= level);
          ...
          <AutoSizer>
            {
              ({ height, width }) => (
                <ArrowKeyStepper
                  rowCount={filteredDisplayObjects.size}
                  columnCount={1}
                  mode="cells"
                    ...
                    ({ onSectionRendered, scrollToRow }) => {
                      return (
                        <Table
                          ...
                          headerHeight={25}
                          rowHeight={40}
                          rowGetter={({ index }) => filteredDisplayObjects.get(index)}
